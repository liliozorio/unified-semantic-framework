...
function atualizacao_Principal(Pessoa $p)
{
    ...
    if (!empty($p->nome) && !is_null($p->nome) && strlen($p->nome) >= 3) {
        $format = 'application/sparql-results+json';
        ...
        foreach ($p as &$item) {
            if ($p->fonte == $item || $p->imagem == $item)
                continue;
            $item = str_replace('"', '\"', html_entity_decode(trim(strtolower($item))));
        }
        ...
        if ($id == -1) {
            ...
            if (strlen($p->apelido) > 0) {
                $endereco = $endereco . $prefix . " foaf:nick \"" . $p->apelido . "\".";
            }
            if (strlen($p->datanasc) > 0) {
                $endereco = $endereco . $prefix . " foaf:birthday \"" . $p->datanasc . "\".";
            }
            if (strlen($p->sexo) > 0) {
                $endereco = $endereco . $prefix . " foaf:gender \"" . $p->sexo . "\".";
            }
            if (strlen($p->imagem) > 0) {
                $endereco = $endereco . $prefix . " foaf:img \"" . $p->imagem . "\".";
            }
            if (strlen($p->idade) > 0) {
                $endereco = $endereco . $prefix . " foaf:age \"" . $p->idade . "\".";
            }
            if (strlen($p->cidade) > 0) {
                $endereco = $endereco . $prefix . " des:cityDes \"" . $p->cidade . "\".";
            }
            if (strlen($p->estado) > 0) {
                $endereco = $endereco . $prefix . " des:stateDes \"" . $p->estado . "\".";
            }
            if (strlen($p->altura) > 0) {
                $endereco = $endereco . $prefix . " dbpprop:height \"" . $p->altura . "\".";
            }
            if (strlen($p->peso) > 0) {
                $endereco = $endereco . $prefix . " dbpprop:weight \"" . $p->peso . "\".";
            }
            if (strlen($p->pele) > 0) {
                $endereco = $endereco . $prefix . " des:skin \"" . $p->pele . "\".";
            }
            if (strlen($p->cor_cabelo) > 0) {
                $endereco = $endereco . $prefix . " dbpprop:hairColor \"" . $p->cor_cabelo . "\".";
            }
            if (strlen($p->cor_olho) > 0) {
                $endereco = $endereco . $prefix . " dbpprop:eyeColor \"" . $p->cor_olho . "\".";
            }
            if (strlen($p->mais_caracteristicas) > 0) {
                $endereco = $endereco . $prefix . " des:moreCharacteristics \"" . $p->mais_caracteristicas . "\".";
            }
            if (strlen($p->data_desaparecimento) > 0) {
                $endereco = $endereco . $prefix . " des:disappearanceDate \"" . $p->data_desaparecimento . "\".";
            }
            if (strlen($p->local_desaparecimento) > 0) {
                $endereco = $endereco . $prefix . " des:disappearancePlace \"" . $p->local_desaparecimento . "\".";
            }
            if (strlen($p->circunstancia_desaparecimento) > 0) {
                $endereco = $endereco . $prefix . " des:circumstanceLocation \"" . $p->circunstancia_desaparecimento . "\".";
            }
            if (strlen($p->data_localizacao) > 0) {
                $endereco = $endereco . $prefix . " des:dateLocation \"" . $p->data_localizacao . "\".";
            }
            if (strlen($p->dados_adicionais) > 0) {
                $endereco = $endereco . $prefix . " des:additionalData \"" . $p->dados_adicionais . "\".";
            }
            if (strlen($p->situacao) > 0) {
                $endereco = $endereco . $prefix . " des:status \"" . $p->situacao . "\".";
            }
            ...
            */
            ...
            echo $where . "<br>";
            preg_match('#(.*)\/#', $p->fonte, $match);
            preg_match('#(.*)\/#', $auxPessoa->fonte, $match2);
            var_dump($match2);
            var_dump($match);
            ...
            if (strlen($NewPessoa->apelido) > 0) {
                $endereco = $endereco . "?s foaf:nick \"" . $NewPessoa->apelido . "\".";
            }
            if (strlen($NewPessoa->datanasc) > 0) {
                $endereco = $endereco . "?s foaf:birthday \"" . $NewPessoa->datanasc . "\".";
            }
            if (strlen($NewPessoa->sexo) > 0) {
                $endereco = $endereco . "?s foaf:gender \"" . $NewPessoa->sexo . "\".";
            }
            if (strlen($NewPessoa->imagem) > 0) {
                $endereco = $endereco . "?s foaf:img \"" . $NewPessoa->imagem . "\".";
            }
            if (strlen($NewPessoa->idade) > 0) {
                $endereco = $endereco . "?s foaf:age \"" . $NewPessoa->idade . "\".";
            }
            if (strlen($NewPessoa->cidade) > 0) {
                $endereco = $endereco . "?s des:cityDes \"" . $NewPessoa->cidade . "\".";
            }
            if (strlen($NewPessoa->estado) > 0) {
                $endereco = $endereco . "?s des:stateDes \"" . $NewPessoa->estado . "\".";
            }
            if (strlen($NewPessoa->altura) > 0) {
                $endereco = $endereco . "?s dbpprop:height \"" . $NewPessoa->altura . "\".";
            }
            if (strlen($NewPessoa->peso) > 0) {
                $endereco = $endereco . "?s dbpprop:weight \"" . $NewPessoa->peso . "\".";
            }
            if (strlen($NewPessoa->pele) > 0) {
                $endereco = $endereco . "?s des:skin \"" . $NewPessoa->pele . "\".";
            }
            if (strlen($NewPessoa->cor_cabelo) > 0) {
                $endereco = $endereco . "?s dbpprop:hairColor \"" . $NewPessoa->cor_cabelo . "\".";
            }
            if (strlen($NewPessoa->cor_olho) > 0) {
                $endereco = $endereco . "?s dbpprop:eyeColor \"" . $NewPessoa->cor_olho . "\".";
            }
            if (strlen($NewPessoa->mais_caracteristicas) > 0) {
                $endereco = $endereco . "?s des:moreCharacteristics \"" . $NewPessoa->mais_caracteristicas . "\".";
            }
            if (strlen($NewPessoa->data_desaparecimento) > 0) {
                $endereco = $endereco . "?s des:disappearanceDate \"" . $NewPessoa->data_desaparecimento . "\".";
            }
            if (strlen($NewPessoa->local_desaparecimento) > 0) {
                $endereco = $endereco . "?s des:disappearancePlace \"" . $NewPessoa->local_desaparecimento . "\".";
            }
            if (strlen($NewPessoa->circunstancia_desaparecimento) > 0) {
                $endereco = $endereco . "?s des:circumstanceLocation \"" . $NewPessoa->circunstancia_desaparecimento . "\".";
            }
            if (strlen($NewPessoa->data_localizacao) > 0) {
                $endereco = $endereco . "?s des:dateLocation \"" . $NewPessoa->data_localizacao . "\".";
            }
            if (strlen($NewPessoa->dados_adicionais) > 0) {
                $endereco = $endereco . "?s des:additionalData \"" . $NewPessoa->dados_adicionais . "\".";
            }
            if (strlen($NewPessoa->situacao) > 0) {
                $endereco = $endereco . "?s des:status \"" . $NewPessoa->situacao . "\".";
            }
